...
export default {
  ...
  methods: {
    addPeriod: function () {
      ...
      let smallerId = sortedQuarters[0]
      let biggerId = sortedQuarters[1]
  ...
  mounted () {
    ...
    this.$on('quarterHovered', id => {
      if (this.clicks === 1) {
        ...
        let sortedQuarters = returnSmallerAndBiggerId(this.firstClickedQuarter, this.secondClickedQuarter)
        let smallerId = sortedQuarters[0]
        let biggerId = sortedQuarters[1]
